...
def main():
    ...
    print(f"   ✓ {len(ngramas_frecuencia)} n-gramas ��nicos encontrados")
    ...
    for ngrama in sorted(ngramas_filtrados.keys()):
        ...
        casos_list = sorted([str(c).strip() for c in ngramas_casos[ngrama]], key=lambda x: int(x) if x.isdigit() else x)
